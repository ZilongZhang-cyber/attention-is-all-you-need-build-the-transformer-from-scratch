...
def run_training_loop_for_steps(batches, parameter_list, model_params, optimizer_state, num_steps, config):
    ...
    for i in range(num_steps):
        step_number = i + 1
        src_batch, tgt_batch = batches[i % len(batches)]
        loss = run_training_step_with_backprop(
            src_batch, tgt_batch, parameter_list, model_params, optimizer_state, step_number, config
        )
        losses.append(loss)
    return losses
...
def select_top_k_candidates(candidate_scores, k):
    ...
    flat_scores = candidate_scores.view(-1)  # (num_beams * vocab_size,)
    top_scores, top_indices = flat_scores.topk(k)

    vocab_size = candidate_scores.size(-1)
    beam_indices = top_indices // vocab_size  # 第几个 beam
    token_ids = top_indices % vocab_size      # 第几个 token
...
def append_tokens_to_beam_sequences(beam_sequences, beam_indices, token_ids):
    ...
    parent_sequences = beam_sequences[beam_indices]  # (k, seq_len) 找到父序列
    new_tokens = token_ids.unsqueeze(-1)             # (k, 1) 新 token 加一维
    return torch.cat([parent_sequences, new_tokens], dim=-1)  # (k, seq_len+1)
